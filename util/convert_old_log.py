...
def convert_all():
    for directory in glob.glob("logs/*"):
        for path in glob.glob(directory + "/*"):
            ...
            new_directory = os.path.join("converted", directory.split(os.sep)[-1])
            new_path = os.path.join(new_directory, "LMS200", new_file_name)
            if os.path.isfile(new_path):
                print("converting lms200 to new format.", new_path)
