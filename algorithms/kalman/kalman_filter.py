...
from . import navpy
from .kalman_constants import constants
...
class KalmanFilterStream(ThreadedStream):
    def __init__(self, initial_roll, initial_pitch, initial_yaw,
                 initial_lat=None, initial_long=None, initial_alt=None, enabled=True, log_level=None,
                 enable_plotting=True):

        super(KalmanFilterStream, self).__init__(enabled, log_level)
        ...
        # self.imu_initialized = not (self.initial_roll is None or self.initial_pitch is None or self.initial_yaw is None)
        self.gps_initialized = not (self.initial_lat is None or self.initial_long is None or self.initial_alt is None)

        self.kf = None

        self.imu_tag = "imu"
        self.imu_service_tag = "kalman-imu"
        self.imu_feed = None

        self.gps_tag = "gps"
        self.gps_service_tag = "kalman-gps"
        self.gps_feed = None

        self.plotter_tag = "plotter"
        self.plotter = None

        self.require_subscription(self.imu_tag, Feed, service_tag=self.imu_service_tag,
                                  required_message_classes=dict)
        self.require_subscription(self.gps_tag, Feed, service_tag=self.gps_service_tag,
                                  required_message_classes=dict)
        self.require_subscription(self.plotter_tag, Subscription, is_suggestion=True)

        self.kf_output = RobotPlot("kalman filter output", enable_plotting)

    def take(self, subscriptions):
        self.imu_feed = self.get_feed(self.imu_tag)
        self.gps_feed = self.get_feed(self.gps_tag)
        if self.is_subscribed(self.plotter_tag):
            self.plotter = self.get_stream(self.plotter_tag)
            self.plotter.add_plots(self.kf_output)

    def init_kf(self):
        self.kf = GrovesKalmanFilter(
    ...
    def run(self):
        while self.is_running():
            if not self.imu_feed.empty():
                while not self.imu_feed.empty():
                    imu_message = self.imu_feed.get()
                    ...
                    if self.gps_initialized:
                        ...
                        self.kf_output.append(*position)

                        output = (position, velocity)
                        self.logger.debug("imu updated: %s" % str(output))
                        self.post(output)

                self.imu_feed.task_done()

            if not self.gps_feed.empty():
                while not self.gps_feed.empty():
                    gps_message = self.gps_feed.get()
                    if self.gps_initialized:
                        self.kf.gps_updated(
                            gps_message["gps_dt"],
                            gps_message["lat"],
                            gps_message["long"],
                            gps_message["altitude"],
                        )
                        position = self.kf.get_position()
                        velocity = self.kf.get_velocity()
                        print(velocity)
                        ...
                        output = (position, velocity)
                        self.logger.debug("gps updated: %s" % str(output))
                        self.post(output)
                    else:
                        self.initial_lat = gps_message["lat"]
                        self.initial_long = gps_message["long"]
                        self.initial_alt = gps_message["altitude"]
                        self.gps_initialized = True
                        self.init_kf()
                self.gps_feed.task_done()
